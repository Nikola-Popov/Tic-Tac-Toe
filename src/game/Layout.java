...
public class Layout extends JPanel {
    private static final int RANDOM_GENERATOR_BOUND = 3;
    ...
    public Layout() {
        ...
        OImageIcon = new ImageIcon("src\\game\\imgs\\tic-tac-toe-O.png");
        XImageIcon = new ImageIcon("src\\game\\imgs\\tic-tac-toe-X.png");
            ...
            for (int cols = 0; cols < gridSize; cols++) {
                ...
                keypad[rows][cols].addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent arg0) {
                        for (int rows = 0; rows < gridSize; rows++) {
                            for (int cols = 0; cols < gridSize; cols++) {
                                int randomPosRows = randomGenerator.nextInt(RANDOM_GENERATOR_BOUND);
                                int randomPosCols = randomGenerator.nextInt(RANDOM_GENERATOR_BOUND);
                                if (keypad[rows][cols].isSelected()) {
                                    if (true) {
                                        ...
                                        if (!keypad[randomPosRows][randomPosCols].isSelected()) {
                                            keypad[randomPosRows][randomPosCols].setIcon(Layout.OImageIcon);
                                            keypad[randomPosRows][randomPosCols].setEnabled(false);
                                            playerSwitcher = false;
                                            playerPos[randomPosRows][randomPosCols] = 'O';
                                            if (victory() == 'o') {
                                                victoryDisplay('o');
                                            } else if (isTheWholeGridSelected() && victory() == 'd') {
                                                victoryDisplay('d');
